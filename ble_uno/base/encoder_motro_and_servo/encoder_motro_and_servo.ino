...
void loop() {
  ...
  if (millis() - g_servo_last_update_time >= kServoUpdateIntervalMs) {
    const bool square = g_codex_pad_frame_decoder.pressed(CodexPadFrameDecoder::Button::kSquareX);
    const bool triangle = g_codex_pad_frame_decoder.pressed(CodexPadFrameDecoder::Button::kTriangleY);
    const bool circle = g_codex_pad_frame_decoder.pressed(CodexPadFrameDecoder::Button::kCircleB);
